...
- タグによる絞り込み
- 全文検索(タイトル・書名・著者・本文を重み付けして検索、`/` キーで検索へ)
- 年別アーカイブ
- お気に入り(記事に印をつけ、お気に入り一覧で読み返せる。ブラウザに保存)
- 記事リンクのコピーによる共有
- ページごとのタイトル・説明文の切り替え(共有・ブックマーク時に内容が伝わる)
- 軽量マークダウンによる本文記述
- 記事を読み進めた割合を示す進捗バー
...
- `src/meta.ts` — ルートごとのページタイトル・説明文
- `src/favorites.ts` — お気に入りの保存と読み出し
...
コメント投稿や他者と共有されるいいねといった、サーバーを介する双方向機能やRSS配信は、サーバーを持たない静的版では扱いません(お気に入りは各自のブラウザ内にのみ保存します)。感想記事はリポジトリのデータとして管理します。
